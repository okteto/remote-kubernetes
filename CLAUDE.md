...
npm test                # Unit tests (Mocha + Chai, plain Node.js)
npm run test:e2e        # E2E tests (launches real VS Code via @vscode/test-electron)
npm run package         # Build .vsix (runs webpack production build first)
npm run ci              # Full CI: install + test + package
```
...
3. `npm run package` — extension packages successfully
...
- **E2E tests use `tsc`, not webpack**: The `test:e2e` script compiles with `tsconfig.test.json` (which has `skipLibCheck: true`). This is required because `@vscode/test-electron` needs individual `.js` files.
...
- ESM imports compiled to CommonJS via webpack
